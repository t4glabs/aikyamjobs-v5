...
// Image migration using axios for proper FormData handling
// Usage: STRAPI_API_TOKEN=your_token node scripts/upload-images-axios.js
...
async function migrateImages() {
  if (!API_TOKEN) {
    ...
    console.log('Run: STRAPI_API_TOKEN=your_token node scripts/upload-images-axios.js\n');
    return;
  }
    ...
    for (let i = 0; i < recentImages.length; i += BATCH_SIZE) {
      ...
      for (const imagePath of batch) {
        try {
          ...
          console.error(`  ✗ Failed: ${path.basename(imagePath)} - ${errorMsg}`);
          failed++;
        }
      }
